...
def fit_Keplerian(t, rv, P_guess, err=None, report=False):
    """
    Parameters
    ----------
    t: 1D array
    ...
    def func(params, t, data, err=None):
        ...
        model = get_RV(P, K, e, tp, w, v0, t, method='rvos')
        if err is None:
            return model - data
        else:
    ...
    params = Parameters()
    params.add('P', value=P_guess, min=P_guess-1, max=P_guess+1.1)
    params.add('e', value=0.2, min=0., max=0.8)
    params.add('tp', value=np.min(t), min=np.min(t), max=np.min(t)+P_guess+1)
    params.add('w', value=np.pi, min=0, max=2. * np.pi)
    params.add('K', value=1, min=0., max=100.)
    params.add('v0', value=0., min=-40., max=40.)
...
# def fit_orbit(system, P_G, periodList, obs_data, niter, n_pls=None, guesses=None, ignorePeriods=None, perThres=None,
#               flag=None):
#     """
...
#     if n_pls is None:
#         n_pls = np.size(system.star.planets)
#
#     # Define objective function: returns the array to be minimized
...
#             params.add('tp' + tag, value=system.star.t0 + (np.random.random() * p_guesses[m]))
...
if __name__ == '__main__':
    ...
    # Fitting planet
    fit_Keplerian(t_test, rv_test_RVOS, P_test)
